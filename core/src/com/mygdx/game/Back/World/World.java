...
public class World {
    ...
    public World(Hero hero) {
        ...
        this.DungeonHub = new Map(50, 90, new TmxMapLoader().load("TMX/Dungeon.tmx"), null, PNJs,
                Gdx.audio.newMusic(Gdx.files.internal("MP3/Dungeon_adventure.mp3")));
        this.Home = new Map(500, 20, new TmxMapLoader().load("TMX/Home.tmx"), Door_list, null,
                Gdx.audio.newMusic(Gdx.files.internal("MP3/battleThemeA.mp3")));
        this.Tavern = new Map(400, 100, new TmxMapLoader().load("TMX/Tavern.tmx"), null, null,
                Gdx.audio.newMusic(Gdx.files.internal("MP3/tavernMusic.mp3")));
    ...
    public Map updateCurrentMap(Map map) {
        if (map != null) {
            if (map.getMusic() != null) {
                map.getMusic().play();
                if (this.CurrentMap.getMusic() != null)
                    this.CurrentMap.getMusic().pause();
            }
            this.CurrentMap = map;
    ...
    public void respawn(Map map) {
        ...
        // Add Wall collisions
        System.out.println("Adding wall collisions ...");
        map.addWalls(false);
    ...
    public void initializeDungeon(int numberOfMaps) {
        ...
        for (int i = 0; i < numberOfMaps + 1; i++) {
            ...
            if (i == 0)
                this.DungeonHub.addDoor(new Door(470, 350, newMap));// DungeonHub is linked to Home
            else if (i == numberOfMaps)
                mapFactory.addDoorBetweenMaps(previousMap, DungeonHub);// Last map is linked back to DungegonHub
            else
                mapFactory.addDoorBetweenMaps(previousMap, newMap);
            ...
            if (i != 0) {
                ...
                // Add the Generated TiledMap into the Map & link it to the previous map
                mapFactory.run(DungeonHubMap.getTiledMap().getTileSets(), TileX, TileY, previousMap);
                ...
                if (i == numberOfMaps) {
                    ...
                    } while (!boss.isValidPosition(bossX, bossY, CurrentMap) || bossX < 2 || bossY < 2
                            || bossY > CurrentMap.getmapWidth() - 2);
                    boss.setX(bossX * 32);
                    boss.setY(bossY * 32);
    ...
    public void IsDungeonFinished() {
        if (CurrentMap == DungeonHub) {
            if (boss.getPV() <= 0) {
                ...
                Hero.setMoney(50);
                disposeDungeon();
                boss = new Boss(0, 0, 100 + 10 * Dungeonlevel, Dungeonlevel * 2, 50, 2, 10, null,
                        new Massue("massue", 50 + 5 * Dungeonlevel, 2));
                boss.setGraphicObject(new GraphicBoss(boss.getName(), DungeonHub.getTiledMap().getTileSets()));
                ...
                for (Map map : Dungeon)
                    map.updatelastposition(map.getX(), map.getY());
            ;
        }
    }
