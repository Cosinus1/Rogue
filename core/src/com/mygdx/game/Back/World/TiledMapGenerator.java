...
public class TiledMapGenerator {
    private final Logger logger = new Logger("MapGen");
    private BlockingQueue<Runnable> requests;
    private Thread thread;
    private TiledMap generatedMap;
    ...
    public TiledMapGenerator(TiledMapTileSets tileSets, int EndX, int EndY) {
    ...
    // Updated generate method with a callback
    public synchronized void generate(Consumer<TiledMap> callback) throws InterruptedException {
        requests.put(new Runnable() {
            @Override
            public void run() {
                generatedMap = createRandomTiledMap();
                if (callback != null) {
                    callback.accept(generatedMap); // Notify once the map is generated
                }
            }
        });
    }

    public synchronized TiledMap getGeneratedMap() {
        return generatedMap;
    }
    ...
    public TiledMap createRandomTiledMap() {
        ...
        // Iterate through the baseLayer to add middleright and middleleft walls to the
        // middleLayer
        for (int x = 0; x < mapWidth; x++) {
    ...
    public void generateRandomWalls(TiledMapTileLayer baseLayer, TiledMapTileSets tileSets, int endX, int endY) {
        ...
        while (i < 10) { // Number of chains
            ...
            // Get an independant copy of baseLayer to check for near walls (we dont want to
            // check walls from current chain)
            backupLayer = cloneLayer(baseLayer);
            // Clone baseLayer (delete too short chains)
            newLayer = cloneLayer(baseLayer);

            // Ensure Wallstarting point is on borders if i< 3 else not too close to the
            // corners
            int breakwhile = 0;
            if (i < 8) {
            ...
            } while ((distanceX + distanceY < 2) && isNearAnotherChain(baseLayer, WallendX, WallendY)
                    && breakwhile < 2000);
            if (breakwhile >= 2000)
                breakcondition = true;
            ...
            if (breakcondition == false) {// Build the Chain if positions are OK
                ...
                while (chainLength < 10) { // Desired chainlength
                    ...
                    if (0 < delta) {
                        switch (signY) {
                            case -1:
                                if (direction == 0) {
                                    if (orientation == -1)
                                        newCell = createCell(createWallTile(tileSets, "base", "down right"));
                                    else
                                        newCell = createCell(createWallTile(tileSets, "base", "left down"));

                                } else
                                    newCell = createCell(createWallTile(tileSets, "base", "vertical"));
                                moveY--;
                                orientation = -1;
                                direction = 1;
                                break;
                            ...
                            case 1:
                                if (direction == 0) {
                                    if (orientation == -1)
                                        newCell = createCell(createWallTile(tileSets, "base", "up right"));
                                    else
                                        newCell = createCell(createWallTile(tileSets, "base", "left up"));
                        ...
                        switch (signX) {
                            case -1:
                                if (direction == 1) {
                                    ...
                                    newCell = createCell(createWallTile(tileSets, "base", "horizontal"));
                                moveX--;
                                orientation = -1;
                                direction = 0;
                                break;
                    ...
                    if (isValidTrajectory(StartX, StartY, endX, endY, newLayer)
                            && isValidPosition(currentX, currentY, baseLayer)) {
                    ...
                    if (distanceX + distanceY == 0 || isNearAnotherChain(backupLayer, currentX, currentY))
                        break;
    ...
    private TiledMapTile createWallTile(TiledMapTileSets tileSets, String level, String orientation) {
        ...
        for (TiledMapTile tile : tileSets.getTileSet("perspective_walls")) {
            if (tile.getProperties().containsKey("wall") && tile.getProperties().get("level").equals(level)
                    && tile.getProperties().get("orientation").equals(orientation)) {
                wallTile = tile;
                break;
            }
        }
    ...
    public TiledMapTileLayer cloneLayer(TiledMapTileLayer Layer) {
        TiledMapTileLayer clonedLayer = new TiledMapTileLayer(Layer.getWidth(), Layer.getHeight(), Layer.getTileWidth(),
                Layer.getTileHeight());
    ...
    public boolean isValidPosition(int X, int Y, TiledMapTileLayer Base) {
        ...
        if (X < 1 || X > (mapWidth) || Y < 1 || Y > (mapHeight - 2))
            return false;
            ...
            for (int y = startY - searchRange; y <= startY + searchRange; y++) {
                if (x > 1 && x < mapWidth && y > 1 && y < mapHeight) {
                    if (baseLayer.getCell(x, y) != null) {
                        if (baseLayer.getCell(x, y).getTile() != null
                                && baseLayer.getCell(x, y).getTile().getProperties().containsKey("wall")
                                && !baseLayer.getCell(x, y).getTile().getProperties().containsKey("border")) {
