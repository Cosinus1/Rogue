...
public class Map {
    ...
    public Map(float x, float y, TiledMap tiledmap, ArrayList<Door> Door_list, ArrayList<Ennemie> NPCs, Music music) {
        ...
        if (tiledmap != null)
            this.collisionLayer = (TiledMapTileLayer) tiledmap.getLayers().get("collisionLayer");
        // Get Map dimensions
        if (tiledmap == null) {
            this.mapHeight = 20;
            this.mapWidth = 30;
    ...
    public boolean insideMap(Object object) {
        float X = object.getX();
        float Y = object.getY();
        if (X < 0 || X > mapWidth * tilewidth || Y < 0 || Y > mapHeight * tileheight)
            return false;
        return true;
    }
    ...
    public void addDoor(Door door) {
        ...
        if (this.Door_list == null)
            this.Door_list = new ArrayList<>();
        this.Door_list.add(door);
    }

    public void addPNJ(Ennemie PNJ) {
        if (this.NPCs == null)
            this.NPCs = new ArrayList<Ennemie>();
        this.NPCs.add(PNJ);
    }
    ...
    public void addWalls(boolean hide) {
        if (wallFactory == null)
            wallFactory = new WallFactory(tiledmap);
    ...
    public void updateElements(float deltaTime) {
        if (Elements != null) {
            ...
            for (Element element : Elements) {
                ...
                if ((element.getSpeedX() != 0 || element.getSpeedY() != 0) && (insideMap(element)))
                    List.add(element);
            }
            Elements.clear();
            Elements = List;
    ...
    public void updateTiledmap(final TiledMap newTiledMap) {
        Gdx.app.postRunnable(new Runnable() {
            @Override
            public void run() {
                tiledmap = newTiledMap;
                // Set collision Layer
                collisionLayer = (TiledMapTileLayer) newTiledMap.getLayers().get("Base");
                if (collisionLayer == null) {
                    System.err.println("No collision Layer found");
                }
                // Set renderer
                renderer = new OrthogonalTiledMapRenderer(newTiledMap);
            }
        });
    }
    ...
    public void createRandomEnnemies(TiledMapTileSets Tilesets, int n) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (buffer > 1000)
                System.err.println("Buffer overloaded : " + buffer);
